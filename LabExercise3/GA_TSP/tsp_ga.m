...
function varargout = tsp_ga(varargin)
    ...
    for iter = 1:numIter
        ...
        %Added standard crossover
        %select the best parent and a random parent
        [minimum,minimum_index] = min(totalDist);
        best_parent = pop(minimum_index,:);
        random_parent = pop(randi(popSize),:);
        
        %select a random crossover point
        crossover_point = randi(n-1);
        %select how many to crossover
        crossover_amount = randi(n-crossover_point);
        %create an empty offspring
        offspring = zeros(1, n);
        
        %copy traits from the best_parent starting from the crossover point
        %to the offspring
        for i=1:crossover_amount
            offspring(1, crossover_point + i) = best_parent(1, crossover_point + i);
        end
        
        %copy the rest to the offspring from the random parent
        difference = setdiff(random_parent, offspring, 'stable');
        j = 1;
        for i=1:length(offspring)
            if (offspring(1,i) == 0)
                offspring(1,i) = difference(1,j);
                j = j+1;
            end
        end
        
        %replace a random individual in the population with the offspring
        pop(randi(popSize),:) = offspring(1,:);
